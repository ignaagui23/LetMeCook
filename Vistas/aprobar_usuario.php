...
$stmt = $conn->prepare("SELECT id, username, email, fecha_creacion, penalizado_hasta FROM usuarios WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['borrar'])) {
        // Primero, podés borrar las recetas del usuario o definir ON DELETE CASCADE en DB
...
<body>
  ...
  <main class="contenedor-recetas">
    <h2>Usuario: <?= htmlspecialchars($usuario['username']) ?></h2>
    ...
    <h3 style="margin-top: 2rem;">Recetas creadas por <?= htmlspecialchars($usuario['username']) ?></h3>
